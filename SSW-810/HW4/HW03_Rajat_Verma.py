...
class Fraction:
    ...
    def gcd(self, num: int, denom: int) -> int:
        """ Function that returns the greatest common factor for two numbers"""
        hcf: int = 0
    ...
    def simplify(self) -> "Fraction":
        """
        Simplify the fraction using the gcf obtained from previous fraction
        """
        if((self.num > 0) and (self.denom < 0)):
            x: int = self.gcd(abs(self.num), abs(self.denom))
            num1: float = -(self.num/x)
            denom1: float = -(self.denom/x)
            return Fraction(num1, denom1)
        elif((self.num < 0) and (self.denom < 0)):
            x = self.gcd(abs(self.num), abs(self.denom))
            numerator1: float = -(self.num/x)
            denominator1: float = -(self.denom/x)
            return Fraction(numerator1, denominator1)
        else:
            x = self.gcd(abs(self.num), abs(self.denom))
            numerator2: float = self.num/x
            denominator2: float = self.denom/x
