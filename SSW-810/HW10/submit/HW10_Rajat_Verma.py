...
from typing import DefaultDict, IO, Tuple, Dict, Iterator, List
import os
...
class Student:
    ...
    """
    field_name: List[str] = ['CWID', 'Name', 'Major', 'Completed Courses','Remaining Required', 'Remaining Electives', 'GPA']
    ...
    def compute_gpa(self) -> float:
        ...
        if len(self._course) == 0:
            return 0.0
        return format(float((sum_gpa)/len(self._course)), '.2f')
    ...
    def info(self)->List[str]:
        ...
        return [self._cwid, self._name, self._major, sorted(self._course .keys()),
                sorted(self._required),sorted(self._electives),self.compute_gpa()]

class Instructor:
    """
        Simulate the behaviour of Instructor in real world
    """
    field_name:List[str]=["Cwid","Name","Department","Course","Students"]
...
def main():
    stevens:Repository=Repository("/home/india/Documents/python/SSW-810/HW10")#use r for eliminating // 
main()